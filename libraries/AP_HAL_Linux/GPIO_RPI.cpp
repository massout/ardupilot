...
// Range based in the first memory address of the first register and the last
// memory addres for the GPIO section (0x7E20'00B4 - 0x7E20'0000).
...
void GPIO_RPI::set_gpio_mode_alt(int pin, int alternative) {
    ...
    const uint8_t tree_bits_position_in_register =
        (pin % pins_per_register) * 3;
    /** Creates a mask to enable the alternative function based in the following
     * logic:
     *
    ...
    // 0b00'000'000'000'000'000'000'ALT'000'000'000 enables alternative for the
    // 4th pin
    const uint32_t mask_with_alt = static_cast<uint32_t>(alternative_value)
                                   << tree_bits_position_in_register;
...
void GPIO_RPI::set_gpio_mode_out(int pin) {
    ...
    // 0b00'000'000'000'000'000'000'001'000'000'000 enables output for the 4th
    // pin
...
void GPIO_RPI::set_gpio_high(int pin) {
    ...
    constexpr uint32_t gpset0_index_value =
        gpset0_memory_offset_value / sizeof(*_gpio);
...
void GPIO_RPI::set_gpio_low(int pin) {
    ...
    constexpr uint32_t gpclr0_index_value =
        gpclr0_memory_offset_value / sizeof(*_gpio);
...
bool GPIO_RPI::get_gpio_logic_state(int pin) {
    ...
    constexpr uint32_t gplev0_index_value =
        gplev0_memory_offset_value / sizeof(*_gpio);
    return _gpio[gplev0_index_value] & (1 << pin);
}

uint32_t GPIO_RPI::get_address(GPIO_RPI::Address address,
                               GPIO_RPI::PeripheralOffset offset) const {
    return static_cast<uint32_t>(address) + static_cast<uint32_t>(offset);
}

volatile uint32_t* GPIO_RPI::get_memory_pointer(uint32_t address,
                                                uint32_t range) const {
    auto pointer =
        mmap(nullptr,  // Any adddress in our space will do
             range,    // Map length
             PROT_READ | PROT_WRITE |
                 PROT_EXEC,  // Enable reading & writing to mapped memory
...
bool GPIO_RPI::openMemoryDevice() {
    _system_memory_device =
        open(_system_memory_device_path, O_RDWR | O_SYNC | O_CLOEXEC);
...
void GPIO_RPI::init() {
    const LINUX_BOARD_TYPE rpi_version =
        UtilRPI::from(hal.util)->detect_linux_board_type();
    ...
    const uint32_t gpio_address =
        get_address(peripheral_base, PeripheralOffset::GPIO);
...
bool GPIO_RPI::usb_connected(void) { return false; }
