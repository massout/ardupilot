...
static UARTDriver* serialDrivers[] = {
    &serial0Driver, &serial1Driver, &serial2Driver, &serial3Driver,
    &serial4Driver, &serial5Driver, &serial6Driver, &serial7Driver,
    &serial8Driver, &serial9Driver,
};
...
static RCInput_Multi rcinDriver{2, new RCInput_AioPRU,
                                new RCInput_RCProtocol(NULL, "/dev/ttyO4")};
#elif CONFIG_HAL_BOARD_SUBTYPE == HAL_BOARD_SUBTYPE_LINUX_NAVIO ||    \
    CONFIG_HAL_BOARD_SUBTYPE == HAL_BOARD_SUBTYPE_LINUX_ERLEBRAIN2 || \
    CONFIG_HAL_BOARD_SUBTYPE == HAL_BOARD_SUBTYPE_LINUX_BH ||         \
    CONFIG_HAL_BOARD_SUBTYPE == HAL_BOARD_SUBTYPE_LINUX_DARK ||       \
...
static RCInput_RCProtocol rcinDriver{NULL, "/dev/ttyS0"};
#elif CONFIG_HAL_BOARD_SUBTYPE == HAL_BOARD_SUBTYPE_LINUX_DISCO
static RCInput_Multi rcinDriver{
    2, new RCInput_RCProtocol("/dev/uart-sbus", "/dev/uart-sumd"),
    new RCInput_UDP()};
...
static RCInput_RCProtocol rcinDriver{"/dev/ttyPS0", NULL};
    ...
    CONFIG_HAL_BOARD_SUBTYPE == HAL_BOARD_SUBTYPE_LINUX_NAVIGATOR
// this is needed to allow for RC input using SERIALn_PROTOCOL=23. No fd is
...
static RCOutput_PCA9685 rcoutDriver(
    i2c_mgr_instance.get_device(1, PCA9685_PRIMARY_ADDRESS), 24576000, 3,
    RPI_GPIO_<27>());
#elif CONFIG_HAL_BOARD_SUBTYPE == HAL_BOARD_SUBTYPE_LINUX_NAVIO
static RCOutput_PCA9685 rcoutDriver(
    i2c_mgr_instance.get_device(1, PCA9685_PRIMARY_ADDRESS), 24576000, 3,
...
static RCOutput_PCA9685 rcoutDriver(
    i2c_mgr_instance.get_device(1, PCA9685_QUATENARY_ADDRESS), 0, 0,
    RPI_GPIO_<4>());
...
#elif CONFIG_HAL_BOARD_SUBTYPE == HAL_BOARD_SUBTYPE_LINUX_BEBOP
static RCOutput_Bebop rcoutDriver(i2c_mgr_instance.get_device(
    HAL_RCOUT_BEBOP_BLDC_I2C_BUS, HAL_RCOUT_BEBOP_BLDC_I2C_ADDR));
#elif CONFIG_HAL_BOARD_SUBTYPE == HAL_BOARD_SUBTYPE_LINUX_DISCO
static RCOutput_Disco rcoutDriver(i2c_mgr_instance.get_device(
    HAL_RCOUT_DISCO_BLDC_I2C_BUS, HAL_RCOUT_DISCO_BLDC_I2C_ADDR));
#elif CONFIG_HAL_BOARD_SUBTYPE == HAL_BOARD_SUBTYPE_LINUX_NAVIO2
...
HAL_Linux::HAL_Linux()
    : AP_HAL::HAL(&serial0Driver, &serial1Driver, &serial2Driver,
                  &serial3Driver, &serial4Driver, &serial5Driver,
                  &serial6Driver, &serial7Driver, &serial8Driver,
                  &serial9Driver, &i2c_mgr_instance, &spi_mgr_instance,
                  &wspi_mgr_instance, &analogIn, &storageDriver, &serial0Driver,
                  &gpioDriver, &rcinDriver, &rcoutDriver, &schedulerInstance,
                  &utilInstance, &opticalFlow, &flashDriver,
#if HAL_WITH_DSP
                  &dspDriver,
#endif
#if HAL_NUM_CAN_IFACES
...
void _usage(void) {
    ...
    printf("\tcpu affinity:\n");
    printf(
        "\t                   --cpu-affinity 1 (single cpu) or 1,3 (multiple "
        "cpus) or 1-3 (range of cpus)\n");
    printf(
        "\t                   -c 1 (single cpu) or 1,3 (multiple cpus) or 1-3 "
        "(range of cpus)\n");
}
...
void HAL_Linux::run(int argc, char* const argv[], Callbacks* callbacks) const {
    ...
    GetOptLong gopt(argc, argv, "A:B:C:D:E:F:G:H:I:J:l:t:s:he:SM:c:", options);
    ...
    while ((opt = gopt.getoption()) != -1) {
        switch (opt) {
            ...
            case 'J': {
                ...
                printf(
                    "WARNING: deprecated option --uart%c/-%c will be removed "
                    "in "
            ...
            case CMDLINE_SERIAL9:
                serialDrivers[opt - CMDLINE_SERIAL0]->set_device_path(
                    gopt.optarg);
                break;
            case 'l':
            ...
            case 'c':
                ...
                Linux::Scheduler::from(scheduler)->set_cpu_affinity(
                    cpu_affinity);
                break;
            case 'h':
                _usage();
...
const AP_HAL::HAL& AP_HAL::get_HAL() { return hal_linux; }

AP_HAL::HAL& AP_HAL::get_HAL_mutable() { return hal_linux; }
