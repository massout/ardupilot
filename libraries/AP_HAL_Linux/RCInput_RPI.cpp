...
#define debug(fmt, args...)                                                   \
    do {                                                                      \
        fprintf(stderr, "[RCInput_RPI]: %s:%d: " fmt, __FUNCTION__, __LINE__, \
                ##args);                                                      \
    } while (0)
#else
#define debug(fmt, args...)
...
#define RCIN_RPI_MAX_COUNTER \
    (RCIN_RPI_BUFFER_LENGTH * PAGE_SIZE * 2)  // 1 circle_buffer
...
#define RCIN_RPI_SIG_LOW  1
static uint16_t RcChnGpioTbl[RCIN_RPI_CHN_NUM] = {
    RPI_GPIO_<5>(),  RPI_GPIO_<6>(),  RPI_GPIO_<13>(), RPI_GPIO_<19>(),
    RPI_GPIO_<26>(), RPI_GPIO_<21>(), RPI_GPIO_<20>(), RPI_GPIO_<16>()};
#else
 ...
 * The PLL channel that used to generate the 100MHz now runs at 750MHz instead
 * of 500, so the fw uses 750/7. see:
 * https://www.raspberrypi.org/forums/viewtopic.php?t=251902
 */
...
Memory_table::Memory_table() { _page_count = 0; }

// Init Memory table
Memory_table::Memory_table(const uint32_t page_count,
                           const LINUX_BOARD_TYPE version) {
    uint32_t i;
    int fdMem, file;
    // Cache coherent adresses depends on RPI's version
    uint32_t bus =
        version == LINUX_BOARD_TYPE::RPI_ZERO_1 ? 0x40000000 : 0xC0000000;
    uint64_t pageInfo;
    void *offset;
    ...
    offset =
        mmap(0, _page_count * PAGE_SIZE, PROT_READ | PROT_WRITE,
    ...
    for (i = 0; i < _page_count; i++) {
        ...
        if (_virt_pages[i] == MAP_FAILED) {
            fprintf(stderr,
                    "Failed to map cache coherent physical page (%m)\n");
            exit(-1);
        }
    ...
    for (i = 0; i < _page_count; i++) {
        ...
        _virt_pages[i] =
            mmap(_virt_pages[i], PAGE_SIZE, PROT_READ | PROT_WRITE,
                 MAP_SHARED | MAP_FIXED | MAP_NORESERVE | MAP_LOCKED, fdMem,
                 ((uintptr_t)_phys_pages[i] &
                  (version == LINUX_BOARD_TYPE::RPI_ZERO_1 ? 0xFFFFFFFFFFFFFFFF
                                                           : ~bus)));
        if (_virt_pages[i] == MAP_FAILED) {
...
// Get virtual address from the corresponding physical address from
// memory_table.
void *Memory_table::get_virt_addr(const uint32_t phys_addr) const {
    // FIXME: Can't the address be calculated directly?
    // FIXME: if the address room  in _phys_pages is not fragmented one may
    // avoid
    //        a complete loop ..
    uint32_t i = 0;
    for (; i < _page_count; i++) {
        if ((uintptr_t)_phys_pages[i] ==
            (((uintptr_t)phys_addr) & 0xFFFFF000)) {
...
uint32_t Memory_table::get_offset(void **const pages,
...
uint32_t Memory_table::bytes_available(const uint32_t read_addr,
                                       const uint32_t write_addr) const {
...
uint32_t Memory_table::get_page_count() const { return _page_count; }
...
void RCInput_RPI::init_dma_cb(dma_cb_t **cbp, uint32_t mode, uint32_t source,
                              uint32_t dest, uint32_t length, uint32_t stride,
                              uint32_t next_cb) {
    (*cbp)->info = mode;
    (*cbp)->src = source;
    (*cbp)->dst = dest;
...
void RCInput_RPI::init_ctrl_data() {
    ...
    /* We are transferring 8 bytes of GPIO register. Every 7th iteration we are
       sampling TIMER register, which length is 8 bytes. So, for every 7 samples
       of GPIO we need 7 * 8 + 8 = 64 bytes of buffer. Value 7 was selected
       specially to have a 64-byte "block" TIMER - GPIO. So, we have integer
       count of such "blocks" at one virtual page. (4096 / 64 = 64 "blocks" per
       page. As minimum, we must have 2 virtual pages of buffer (to have integer
       count of vitual pages for control blocks): for every 7 iterations (64
       bytes of buffer) we need 7 control blocks for GPIO sampling, 7 control
       blocks for setting frequency and 1 control block for sampling timer, so,
       we need 7 + 7 + 1 = 15 control blocks. For integer value, we need 15
       pages of control blocks. Each control block length is 32 bytes. In 15
       pages we will have (15 * 4096 / 32) = 15 * 128 control blocks. 15 * 128
       control blocks = 64 * 128 bytes of buffer = 2 pages of buffer. So, for 7
       * 64 * 2 iteration we init DMA for sampling GPIO and timer to ((7 * 8 +
       8) * 64 * 2) = 8192 bytes = 2 pages of buffer.
    */
    ...
    for (uint32_t i = 0; i < 7 * 128 * RCIN_RPI_BUFFER_LENGTH; i++) {
        ...
        if (i % 7 == 0) {
            ...
            init_dma_cb(&cbp_curr,
                        RCIN_RPI_DMA_NO_WIDE_BURSTS | RCIN_RPI_DMA_WAIT_RESP,
                        RCIN_RPI_TIMER_BASE,
                        (uintptr_t)circle_buffer->get_page(
                            circle_buffer->_phys_pages, dest),
                        8, 0,
                        (uintptr_t)con_blocks->get_page(
                            con_blocks->_phys_pages, cbp + sizeof(dma_cb_t)));
            dest += 8;
            cbp += sizeof(dma_cb_t);
        }
        ...
        /* DMA is waiting data request signal (DREQ) from PCM. PCM is set
           for 1.25 MhZ freqency, so, each sample of GPIO is limited by writing
           to PCA queue.
        */
        cbp_curr =
            (dma_cb_t *)con_blocks->get_page(con_blocks->_virt_pages, cbp);
        init_dma_cb(&cbp_curr,
                    RCIN_RPI_DMA_NO_WIDE_BURSTS | RCIN_RPI_DMA_WAIT_RESP |
                        RCIN_RPI_DMA_D_DREQ | RCIN_RPI_DMA_PER_MAP(2),
                    RCIN_RPI_TIMER_BASE, phys_fifo_addr, 4, 0,
                    (uintptr_t)con_blocks->get_page(con_blocks->_phys_pages,
                                                    cbp + sizeof(dma_cb_t)));
    ...
    cbp -= sizeof(dma_cb_t);
    ((dma_cb_t *)con_blocks->get_page(con_blocks->_virt_pages, cbp))->next =
        (uintptr_t)con_blocks->get_page(con_blocks->_phys_pages, 0);
}
...
void RCInput_RPI::init_PCM() {
    ...
    if (_version != LINUX_BOARD_TYPE::RPI_4) {
        clk_reg[RCIN_RPI_PCMCLK_DIV] =
            0x5A000000 | ((RCIN_RPI_PLL_CLK / RCIN_RPI_SAMPLE_FREQ)
                          << 12);  // Set pcm div for BCM2835 500MHZ clock. If
                                   // we need to configure DMA frequency.
    } else {
        // RPI-4
        clk_reg[RCIN_RPI_PCMCLK_DIV] =
            0x5A000000 | ((RCIN_RPI4_PLL_CLK / RCIN_RPI_SAMPLE_FREQ)
                          << 12);  // Set pcm div for BCM2711 700MHz clock. If
                                   // we need to configure DMA frequency.
    }
    hal.scheduler->delay_microseconds(100);
    clk_reg[RCIN_RPI_PCMCLK_CNTL] = 0x5A000016;  // Source=PLLD and enable
    hal.scheduler->delay_microseconds(100);
    pcm_reg[RCIN_RPI_PCM_TXC_A] =
        0 << 31 | 1 << 30 | 0 << 20 | 0 << 16;  // 1 channel, 8 bits
    hal.scheduler->delay_microseconds(100);
    pcm_reg[RCIN_RPI_PCM_MODE_A] = (10 - 1)
                                   << 10;  // PCM mode 0b0010010000000000
    hal.scheduler->delay_microseconds(100);
    pcm_reg[RCIN_RPI_PCM_CS_A] |= 1 << 4 | 1 << 3;  // Clear FIFOs
    hal.scheduler->delay_microseconds(100);
    pcm_reg[RCIN_RPI_PCM_DREQ_A] =
...
void RCInput_RPI::init_DMA() {
    dma_reg[RCIN_RPI_DMA_CS | RCIN_RPI_DMA_CHANNEL << 8] =
        RCIN_RPI_DMA_RESET;  // Reset DMA
    hal.scheduler->delay_microseconds(100);
    dma_reg[RCIN_RPI_DMA_CS | RCIN_RPI_DMA_CHANNEL << 8] =
        RCIN_RPI_DMA_INT | RCIN_RPI_DMA_END;
    dma_reg[RCIN_RPI_DMA_CONBLK_AD | RCIN_RPI_DMA_CHANNEL << 8] =
        reinterpret_cast<uintptr_t>(con_blocks->get_page(
            con_blocks->_phys_pages, 0));  // Set first control block address
    dma_reg[RCIN_RPI_DMA_DEBUG | RCIN_RPI_DMA_CHANNEL << 8] =
        7;  // clear debug error flags
    dma_reg[RCIN_RPI_DMA_CS | RCIN_RPI_DMA_CHANNEL << 8] =
...
void RCInput_RPI::teardown() { stop_dma(); }
...
void RCInput_RPI::init() {
    ...
    circle_buffer =
        NEW_NOTHROW Memory_table(RCIN_RPI_BUFFER_LENGTH * 2, _version);
    con_blocks =
        NEW_NOTHROW Memory_table(RCIN_RPI_BUFFER_LENGTH * 15, _version);
    ...
    curr_tick = *((uint64_t *)circle_buffer->get_page(
        circle_buffer->_virt_pages, curr_pointer));
    curr_pointer += 8;
    signal_states = *((uint64_t *)circle_buffer->get_page(
        circle_buffer->_virt_pages, curr_pointer));
    for (uint32_t i = 0; i < RCIN_RPI_CHN_NUM; ++i) {
        rc_channels[i].prev_tick = curr_tick;
        rc_channels[i].curr_signal = (signal_states & (1 << RcChnGpioTbl[i]))
                                         ? RCIN_RPI_SIG_HIGH
                                         : RCIN_RPI_SIG_LOW;
...
void RCInput_RPI::_timer_tick() {
    ...
    dma_cb_t *ad = (dma_cb_t *)con_blocks->get_virt_addr(
        dma_reg[RCIN_RPI_DMA_CONBLK_AD | RCIN_RPI_DMA_CHANNEL << 8]);
    ...
    const uint32_t offset =
        con_blocks->get_offset(con_blocks->_virt_pages, (uintptr_t)ad);
    for (int j = 1; j >= -1; j--) {
        // Get address of next or previous (dma_cb_t)
        ad = (dma_cb_t *)con_blocks->get_page(
            con_blocks->_virt_pages, offset + (uint32_t)(sizeof(dma_cb_t) * j));
        if (!ad) {
            continue;
        }
        ...
        if (x != nullptr) {
            counter = circle_buffer->bytes_available(
                curr_pointer, circle_buffer->get_offset(
                                  circle_buffer->_virt_pages, (uintptr_t)x));
            break;
        }
    }
    ...
    for (; counter > 0x40;) {
        ...
        signal_states = *((uint64_t *)circle_buffer->get_page(
            circle_buffer->_virt_pages, curr_pointer));
        for (uint32_t i = 0; i < RCIN_RPI_CHN_NUM; ++i) {
            rc_channels[i].curr_signal =
                (signal_states & (1 << RcChnGpioTbl[i])) ? RCIN_RPI_SIG_HIGH
                                                         : RCIN_RPI_SIG_LOW;

            // If the signal changed
            if (rc_channels[i].curr_signal != rc_channels[i].last_signal) {
                rc_channels[i].delta_time =
                    curr_tick - rc_channels[i].prev_tick;
                rc_channels[i].prev_tick = curr_tick;
                switch (rc_channels[i].state) {
                    ...
                    case RCIN_RPI_ZERO_STATE:
                        if (rc_channels[i].curr_signal == 0) {
                            rc_channels[i].width_s0 =
                                (uint16_t)rc_channels[i].delta_time;
