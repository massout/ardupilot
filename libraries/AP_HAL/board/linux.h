...
#define PROBE_IMU_I2C(driver, bus, addr, args...)  \
    ADD_BACKEND(AP_InertialSensor_##driver::probe( \
        *this, GET_I2C_DEVICE(bus, addr), ##args))
#define PROBE_IMU_I2C2(driver, bus, addr1, addr2, args...) \
    ADD_BACKEND(AP_InertialSensor_##driver::probe(         \
        *this, hal.i2c_mgr->get_device(bus, addr1),        \
        hal.i2c_mgr->get_device(bus, addr2), ##args))
#define PROBE_IMU_SPI(driver, devname, args...)    \
    ADD_BACKEND(AP_InertialSensor_##driver::probe( \
        *this, hal.spi->get_device(devname), ##args))
#define PROBE_IMU_SPI2(driver, devname1, devname2, args...)                  \
    ADD_BACKEND(AP_InertialSensor_##driver::probe(                           \
        *this, hal.spi->get_device(devname1), hal.spi->get_device(devname2), \
        ##args))

#define PROBE_BARO_I2C(driver, bus, addr, args...) \
    ADD_BACKEND(AP_Baro_##driver::probe(           \
        *this, std::move(GET_I2C_DEVICE(bus, addr)), ##args))
#define PROBE_BARO_SPI(driver, devname, args...) \
    ADD_BACKEND(AP_Baro_##driver::probe(         \
        *this, std::move(hal.spi->get_device(devname)), ##args))

#define PROBE_MAG_I2C(driver, bus, addr, args...) \
    ADD_BACKEND(DRIVER_##driver,                  \
                AP_Compass_##driver::probe(GET_I2C_DEVICE(bus, addr), ##args))
#define PROBE_MAG_SPI(driver, devname, args...)              \
    ADD_BACKEND(DRIVER_##driver, AP_Compass_##driver::probe( \
                                     hal.spi->get_device(devname), ##args))
#define PROBE_MAG_IMU(driver, imudev, imu_instance, args...) \
    ADD_BACKEND(DRIVER_##driver,                             \
                AP_Compass_##driver::probe_##imudev(imu_instance, ##args))
#define PROBE_MAG_IMU_I2C(driver, imudev, bus, addr, args...)         \
    ADD_BACKEND(DRIVER_##driver, AP_Compass_##driver::probe_##imudev( \
                                     GET_I2C_DEVICE(bus, addr), ##args))
...
#define HAL_INS_PROBE_LIST \
    PROBE_IMU_SPI(Invensense, "mpu9250", ROTATION_ROLL_180_YAW_270)
#define HAL_INS_PROBE_LIST \
    PROBE_IMU_SPI(Invensense, "mpu9250ext", ROTATION_NONE)
#define HAL_MAG_PROBE_LIST PROBE_MAG_IMU(AK8963, mpu9250, 0, ROTATION_NONE)
#define HAL_MAG_PROBE_LIST \
    PROBE_MAG_IMU(AK8963, mpu9250, 1, ROTATION_ROLL_180_YAW_90)
#define HAL_BOARD_LOG_DIRECTORY     "/home/pi/SHARKY/ardupilot/logs"
#define HAL_BOARD_TERRAIN_DIRECTORY "/home/pi/SHARKY/ardupilot/terrain"
#define HAL_BOARD_STORAGE_DIRECTORY "/home/pi/SHARKY/ardupilot"
#define HAL_PARAM_DEFAULTS_PATH     "/home/pi/SHARKY/sharky.parm"
...
#else
#define HAL_INS_PROBE_LIST \
    PROBE_IMU_SPI(Invensense, "mpu9250", ROTATION_ROLL_180_YAW_90)
#endif
...
#define HAL_BARO_PROBE_LIST \
    PROBE_BARO_I2C(MS56XX, 1, 0x77, AP_Baro_MS56XX::BARO_MS5607)
...
#define HAL_BOARD_STORAGE_DIRECTORY "/data/ftp/internal_000/ardupilot"
#define HAL_INS_PROBE_LIST \
    PROBE_IMU_I2C(Invensense, 2, 0x68, ROTATION_PITCH_180_YAW_90)
#define HAL_MAG_PROBE_LIST PROBE_MAG_I2C(AK8963, 1, 0x0d, ROTATION_NONE)
#define HAL_BARO_PROBE_LIST \
    PROBE_BARO_I2C(MS56XX, 1, 0x77, AP_Baro_MS56XX::BARO_MS5607)
#define HAL_UTILS_HEAT                             HAL_LINUX_HEAT_PWM
...
#define HAL_INS_PROBE_LIST \
    PROBE_IMU_SPI(Invensense, "mpu9250", ROTATION_YAW_270)
#define HAL_BARO_PROBE_LIST PROBE_BARO_SPI(MS56XX, "ms5611")
#define HAL_MAG_PROBE_LIST  PROBE_MAG_IMU(AK8963, mpu9250, 0, ROTATION_NONE)
#define HAL_PROBE_EXTERNAL_I2C_COMPASSES
...
#define HAL_INS_PROBE_LIST \
    PROBE_IMU_SPI(Invensense, "icm20602", ROTATION_ROLL_180_YAW_270)
#define HAL_MAG_PROBE1 \
    PROBE_MAG_SPI(MMC5XX3, "mmc5983", false, ROTATION_YAW_180)
...
#define HAL_MAG_PROBE_LIST \
    PROBE_MAG_IMU_I2C(AK8963, mpu9250, 2, 0x0c, ROTATION_NONE)
#define HAL_PROBE_EXTERNAL_I2C_COMPASSES
...
#define HAL_MAG_PROBE3 \
    PROBE_MAG_I2C(IST8310, 4, 0x0e, true, ROTATION_PITCH_180_YAW_90)
#define HAL_MAG_PROBE_LIST \
    HAL_MAG_PROBE1;        \
    HAL_MAG_PROBE2;        \
    HAL_MAG_PROBE3
...
#define HAL_INS_PROBE_LIST                                                 \
    PROBE_IMU_SPI2(Invensense, "rst_g", "rst_a", ROTATION_ROLL_180_YAW_90, \
                   ROTATION_ROLL_180_YAW_90)
#define HAL_BARO_PROBE_LIST PROBE_BARO_SPI(MS56XX, "ms5611")
#define HAL_MAG_PROBE_LIST \
    PROBE_MAG_SPI(LIS3MDL, lis3mdl, false, ROTATION_ROLL_180_YAW_90)
...
// #define HAL_MAG_PROBE_LIST PROBE_MAG_I2C(QMC5883L, 1, 0x0d,true ,
// ROTATION_NONE)
