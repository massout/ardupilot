...
AP_CRSF_Telem::AP_CRSF_Telem() : AP_RCTelemetry(0) { singleton = this; }

AP_CRSF_Telem::~AP_CRSF_Telem(void) { singleton = nullptr; }

bool AP_CRSF_Telem::init(void) {
    // sanity check that we are using a UART for RC input
    if (!AP::serialmanager().have_serial(AP_SerialManager::SerialProtocol_RCIN,
                                         0) &&
        !AP::serialmanager().have_serial(AP_SerialManager::SerialProtocol_CRSF,
                                         0)) {
        return false;
    }

    // Someone explicitly configure CRSF control for VTX
    if (AP::serialmanager().have_serial(AP_SerialManager::SerialProtocol_CRSF,
                                        0)) {
...
void AP_CRSF_Telem::setup_wfq_scheduler(void) {
    ...
    add_scheduler_entry(5, 20);  // parameters       50Hz (generally not active
                                 // unless requested by the TX)
    ...
    add_scheduler_entry(5, 20);  // command          50Hz (generally not active
                                 // unless requested by the TX)
    add_scheduler_entry(5,
                        500);  // version ping      2Hz (only active at startup)
    add_scheduler_entry(5, 100);  // device ping      10Hz (only active during
                                  // TX loss, also see CRSF_RX_TIMEOUT)
...
void AP_CRSF_Telem::setup_custom_telemetry() {
    ...
    const int8_t frsky_port = AP::serialmanager().find_portnum(
        AP_SerialManager::SerialProtocol_FrSky_SPort_Passthrough, 0);
    if (frsky_port != -1) {
        GCS_SEND_TEXT(MAV_SEVERITY_CRITICAL,
                      "%s: passthrough telemetry conflict on SERIAL%d",
                      get_protocol_string(), frsky_port);
    ...
    passthrough->set_scheduler_entry_min_period(
        AP_Frsky_SPort_Passthrough::ATTITUDE, 350);  // 3Hz
    ...
    GCS_SEND_TEXT(MAV_SEVERITY_DEBUG, "%s: custom telem init done, fw %d.%02d",
                  get_protocol_string(), _crsf_version.major,
                  _crsf_version.minor);

    _custom_telem.init_done = true;
}

void AP_CRSF_Telem::update_custom_telemetry_rates(
...
bool AP_CRSF_Telem::process_rf_mode_changes() {
...
#if !defined(STM32H7) && defined(HAL_BOARD_CHIBIOS)
    // warn the user if their setup is sub-optimal, H7 does not need DMA on
    // serial port
    if (_telem_bootstrap_msg_pending && !uart->is_dma_enabled()) {
        GCS_SEND_TEXT(MAV_SEVERITY_WARNING,
                      "%s: running on non-DMA serial port",
                      get_protocol_string());
    }
#endif
    // note if option was set to show LQ in place of RSSI
    bool current_lq_as_rssi_active =
        rc().option_is_enabled(RC_Channels::Option::USE_CRSF_LQ_AS_RSSI);
    if (_telem_bootstrap_msg_pending ||
        _noted_lq_as_rssi_active != current_lq_as_rssi_active) {
        _noted_lq_as_rssi_active = current_lq_as_rssi_active;
        GCS_SEND_TEXT(MAV_SEVERITY_INFO, "%s: RSSI now displays %s",
                      get_protocol_string(),
                      current_lq_as_rssi_active ? " as LQ" : "normally");
    }
    ...
    if ((now - _telem_last_report_ms > 5000)) {
        // report an RF mode change or a change in telemetry rate if we haven't
        // done so in the last 5s
        if (!rc().option_is_enabled(
                RC_Channels::Option::SUPPRESS_CRSF_MESSAGE) &&
            (_telem_rf_mode != current_rf_mode ||
             abs(int16_t(_telem_last_avg_rate) -
                 int16_t(_scheduler.avg_packet_rate)) > 25)) {
            GCS_SEND_TEXT(MAV_SEVERITY_INFO,
                          "%s: Link rate %dHz, Telemetry rate %dHz",
                          get_protocol_string(),
                          crsf->get_link_rate(_crsf_version.protocol),
                          get_telemetry_rate());
        }
        ...
        if (_telem_last_report_ms ==
            0) {  // only want to show bootstrap messages once
...
uint8_t AP_CRSF_Telem::get_custom_telem_frame_id() const {
    if (!_crsf_version.pending &&
        ((_crsf_version.major > 4 ||
          (_crsf_version.major == 4 && _crsf_version.minor >= 6)) ||
         is_elrs())) {
...
AP_RCProtocol_CRSF::RFMode AP_CRSF_Telem::get_rf_mode() const {
    ...
    if (!_crsf_version.pending && _crsf_version.use_rf_mode) {
        if (is_elrs()) {
            return static_cast<AP_RCProtocol_CRSF::RFMode>(
                uint8_t(AP_RCProtocol_CRSF::RFMode::CRSF_RF_MAX_MODES) +
                crsf->get_link_status().rf_mode);
        }
        return static_cast<AP_RCProtocol_CRSF::RFMode>(
            crsf->get_link_status().rf_mode);
    } else if (is_tracer()) {
...
bool AP_CRSF_Telem::is_high_speed_telemetry(
    const AP_RCProtocol_CRSF::RFMode rf_mode) const {
    if (_crsf_version.protocol !=
        AP_RCProtocol_CRSF::ProtocolType::PROTOCOL_ELRS) {
        return rf_mode == AP_RCProtocol_CRSF::RFMode::CRSF_RF_MODE_150HZ ||
               rf_mode == AP_RCProtocol_CRSF::RFMode::CRSF_RF_MODE_250HZ;
    }
...
void AP_CRSF_Telem::disable_tx_entries() {
    ...
    // GENERAL_COMMAND and PARAMETERS will only be sent under very specific
    // circumstances
}
...
void AP_CRSF_Telem::adjust_packet_weight(bool queue_empty) {
    ...
    */
    ...
    if (!_custom_telem.params_mode_active && _pending_request.frame_type > 0 &&
        _pending_request.frame_type !=
            AP_RCProtocol_CRSF::CRSF_FRAMETYPE_PARAM_DEVICE_INFO &&
    ...
    switch (idx) {
        ...
        case VTX_PARAMETERS:
            ...
            return AP::vtx().have_params_changed() ||
                   _vtx_power_change_pending || _vtx_freq_change_pending ||
                   _vtx_options_change_pending;
#else
            return false;
#endif
        case PASSTHROUGH:
            return rc().option_is_enabled(
                RC_Channels::Option::CRSF_CUSTOM_TELEMETRY);
        case STATUS_TEXT:
            return rc().option_is_enabled(
                       RC_Channels::Option::CRSF_CUSTOM_TELEMETRY) &&
                   !queue_empty;
        ...
        case VERSION_PING:
            return _crsf_version.pending &&
                   AP::crsf()->is_detected();  // only send pings if protocol
                                               // has been detected
        case HEARTBEAT:
            return true;  // always send heartbeat if enabled
        case DEVICE_PING:
            return !_crsf_version.pending;  // only send pings if version has
                                            // been negotiated
        default:
    ...
    switch (idx) {
        ...
        case PASSTHROUGH:
            if (is_high_speed_telemetry(_telem_rf_mode)) {
                ...
                const uint8_t size =
                    is_elrs() ? 3
                              : AP_CRSF_Telem::
                                    PASSTHROUGH_MULTI_PACKET_FRAME_MAX_SIZE;
        ...
        case VERSION_PING:
            ...
            if (_crsf_version.retry_count++ > CRSF_RX_DEVICE_PING_MAX_RETRY) {
                ...
                GCS_SEND_TEXT(MAV_SEVERITY_DEBUG, "%s: RX device ping failed",
                              get_protocol_string());
            } else {
                calc_device_ping(
                    AP_RCProtocol_CRSF::CRSF_ADDRESS_CRSF_RECEIVER);
...
bool AP_CRSF_Telem::_process_frame(AP_RCProtocol_CRSF::FrameType frame_type,
                                   void* data) {
    switch (frame_type) {
        ...
        // the EVO sends battery frames and we should send telemetry back to
        // populate the OSD
...
void AP_CRSF_Telem::process_vtx_frame(VTXFrame* vtx) {
    ...
    debug(
        "VTX: SmartAudio: %d, Avail: %d, FreqMode: %d, Band: %d, Channel: %d, "
        "Freq: %d, PitMode: %d, Pwr: %d, Pit: %d",
        vtx->smart_audio_ver, vtx->is_vtx_available,
        vtx->is_in_user_frequency_mode, vtx->band, vtx->channel,
        vtx->is_in_user_frequency_mode
            ? vtx->user_frequency
            : AP_VideoTX::get_frequency_mhz(vtx->band, vtx->channel),
    ...
    if (vtx->is_in_pitmode) {
        apvtx.set_options(apvtx.get_options() |
                          uint8_t(AP_VideoTX::VideoOptions::VTX_PITMODE));
    } else {
        apvtx.set_options(apvtx.get_options() &
                          ~uint8_t(AP_VideoTX::VideoOptions::VTX_PITMODE));
    }
    // make sure the configured values now reflect reality
    if (!apvtx.set_defaults() &&
        (_vtx_power_change_pending || _vtx_freq_change_pending ||
         _vtx_options_change_pending)) {
...
void AP_CRSF_Telem::process_vtx_telem_frame(VTXTelemetryFrame* vtx) {
    ...
    debug("VTXTelemetry: Freq: %d, PitMode: %d, Power: %d", vtx->frequency,
          vtx->pitmode, vtx->power);
...
void AP_CRSF_Telem::process_ping_frame(ParameterPingFrame* ping) {
    ...
    if (ping->destination != 0 &&
        ping->destination !=
            AP_RCProtocol_CRSF::CRSF_ADDRESS_FLIGHT_CONTROLLER) {
...
void AP_CRSF_Telem::process_device_info_frame(ParameterDeviceInfoFrame* info) {
    debug("process_device_info_frame: 0x%x -> 0x%x", info->origin,
          info->destination);
    ...
    if (info->origin != 0 &&
        info->origin != AP_RCProtocol_CRSF::CRSF_ADDRESS_CRSF_RECEIVER) {
        return;
    }
    /*
        Payload size is 58:
    ...
    */
    ...
    if (strncmp((char*)info->payload, "Tracer", 6) == 0) {
        _crsf_version.protocol =
            AP_RCProtocol_CRSF::ProtocolType::PROTOCOL_TRACER;
    ...
    if (!is_elrs()) {
        /*
            fw major ver = offset + terminator (8bits) + serial (32bits) + hw id
           (32bits) + 3rd byte of sw id = 11bytes fw minor ver = offset +
           terminator (8bits) + serial (32bits) + hw id (32bits) + 4th byte of
           sw id = 12bytes
        */
        _crsf_version.major = info->payload[offset + 11];
        _crsf_version.minor = info->payload[offset + 12];
    } else {
        // ELRS does not populate the version field so cook up something
        // sensible
    ...
    if (is_elrs() || (!is_tracer() && (_crsf_version.major > 3 ||
                                       (_crsf_version.major == 3 &&
                                        _crsf_version.minor >= 72)))) {
...
void AP_CRSF_Telem::process_command_frame(CommandFrame* command) {
    debug("process_command_frame: 0x%x -> 0x%x: 0x%x", command->origin,
          command->destination, command->payload[0]);
    ...
    if (command->origin != 0 &&
        command->origin != AP_RCProtocol_CRSF::CRSF_ADDRESS_CRSF_RECEIVER) {
        return;
    }

    switch (command->payload[0]) {
        case AP_RCProtocol_CRSF::CRSF_COMMAND_GENERAL_CRSF_SPEED_PROPOSAL: {
            uint32_t baud_rate = command->payload[2] << 24 |
                                 command->payload[3] << 16 |
...
void AP_CRSF_Telem::process_param_read_frame(
    ParameterSettingsReadFrame* read_frame) {
    debug("process_param_read_frame: %d -> %d for %d[%d]", read_frame->origin,
          read_frame->destination, read_frame->param_num,
          read_frame->param_chunk);
    if (read_frame->destination != 0 &&
...
void AP_CRSF_Telem::update_vtx_params() {
    ...
    // This function does ugly things with the vtx parameters which will upset
    // other providers
    if (!vtx.get_enabled() ||
        !vtx.is_provider_enabled(AP_VideoTX::VTXType::CRSF)) {
        return;
    }

    _vtx_freq_change_pending = vtx.update_band() || vtx.update_channel() ||
                               vtx.update_frequency() ||
                               _vtx_freq_change_pending;
    // don't update the power if we are supposed to be in pitmode as this will
    // take us out of pitmode
    const bool pitmode = vtx.get_configured_options() &
                         uint8_t(AP_VideoTX::VideoOptions::VTX_PITMODE);
    _vtx_power_change_pending =
        !pitmode && (vtx.update_power() || _vtx_power_change_pending);
    _vtx_options_change_pending =
        vtx.update_options() || _vtx_options_change_pending;
    ...
    if (_vtx_freq_change_pending || _vtx_power_change_pending ||
        ...
        debug(
            "update_params(): freq %d->%d, chan: %d->%d, band: %d->%d, pwr: "
            "%d->%d, opts: %d->%d",
            vtx.get_frequency_mhz(), vtx.get_configured_frequency_mhz(),
            vtx.get_channel(), vtx.get_configured_channel(), vtx.get_band(),
            vtx.get_configured_band(), vtx.get_power_mw(),
            vtx.get_configured_power_mw(), vtx.get_options(),
            vtx.get_configured_options());

        _telem_type = AP_RCProtocol_CRSF::CRSF_FRAMETYPE_COMMAND;
        _telem.ext.command.destination = AP_RCProtocol_CRSF::CRSF_ADDRESS_VTX;
        _telem.ext.command.origin =
            AP_RCProtocol_CRSF::CRSF_ADDRESS_FLIGHT_CONTROLLER;
        _telem.ext.command.command_id = AP_RCProtocol_CRSF::CRSF_COMMAND_VTX;

        uint8_t len = 5;
        // prioritize option changes so that the pilot can get in and out of
        // pitmode
        if (_vtx_options_change_pending) {
            _telem.ext.command.payload[0] =
                AP_RCProtocol_CRSF::CRSF_COMMAND_VTX_PITMODE;
            if (vtx.get_configured_options() &
                uint8_t(AP_VideoTX::VideoOptions::VTX_PITMODE)) {
...
#pragma GCC diagnostic ignored "-Wstringop-overflow"
        ...
        uint8_t crc =
            crc8_dvb(0, AP_RCProtocol_CRSF::CRSF_FRAMETYPE_COMMAND, 0xBA);
...
void AP_CRSF_Telem::calc_battery() {
    ...
    _telem.bcast.battery.voltage =
        htobe16(uint16_t(roundf(_battery.voltage(0) * 10.0f)));
...
void AP_CRSF_Telem::calc_gps() {
    const Location& loc = AP::gps().location(
        0);  // use the first gps instance (same as in send_mavlink_gps_raw)

    _telem.bcast.gps.latitude = htobe32(loc.lat);
    _telem.bcast.gps.longitude = htobe32(loc.lng);
    _telem.bcast.gps.groundspeed =
        htobe16(roundf(AP::gps().ground_speed() * 100000 / 3600));
    _telem.bcast.gps.altitude =
        htobe16(constrain_int16(loc.alt / 100, 0, 5000) + 1000);
    _telem.bcast.gps.gps_heading =
        htobe16(roundf(AP::gps().ground_course() * 100.0f));
...
void AP_CRSF_Telem::calc_attitude() {
    ...
    _telem.bcast.attitude.roll_angle = htobe16(constrain_int16(
        roundf(wrap_PI(_ahrs.get_roll()) * 10000.0f), -INT_PI, INT_PI));
    _telem.bcast.attitude.pitch_angle = htobe16(constrain_int16(
        roundf(wrap_PI(_ahrs.get_pitch()) * 10000.0f), -INT_PI, INT_PI));
    _telem.bcast.attitude.yaw_angle = htobe16(constrain_int16(
        roundf(wrap_PI(_ahrs.get_yaw()) * 10000.0f), -INT_PI, INT_PI));
...
void AP_CRSF_Telem::calc_flight_mode() {
    ...
    if (notify) {
        ...
        hal.util->snprintf(
            ...
            sizeof(AP_CRSF_Telem::FlightModeFrame), "%s%s",
            notify->get_flight_mode_str(),
            rc().option_is_enabled(RC_Channels::Option::CRSF_FM_DISARM_STAR) &&
                    !hal.util->get_soft_armed()
                ? "*"
                : "");
        // Note: strlen(_telem.bcast.flightmode.flight_mode) is safe because
        // called on a guaranteed null terminated string
        _telem_size = strlen(_telem.bcast.flightmode.flight_mode) +
                      1;  // send the terminator as well
...
void AP_CRSF_Telem::calc_device_info() {
    ...
    // write out the name with version, max width is 60 - 18 = the meaning of
    // life
    ...
    put_be32_ptr(&_telem.ext.info.payload[n],  // serial number
                 uint32_t(fwver.major) << 24 | uint32_t(fwver.minor) << 16 |
                     uint32_t(fwver.patch) << 8 | uint32_t(fwver.fw_type));
    n += 4;
    put_be32_ptr(&_telem.ext.info.payload[n],  // hardware id
                 uint32_t(fwver.vehicle_type) << 24 |
                     uint32_t(fwver.board_type) << 16 |
                     uint32_t(fwver.board_subtype));
    n += 4;
    put_be32_ptr(&_telem.ext.info.payload[n],
                 fwver.os_sw_version);  // software id
    n += 4;
#if OSD_PARAM_ENABLED
    _telem.ext.info.payload[n++] = AP_OSD_ParamScreen::NUM_PARAMS *
                                   AP_OSD_NUM_PARAM_SCREENS;  // param count
#else
...
void AP_CRSF_Telem::calc_command_response() {
    _telem.ext.command.destination =
        AP_RCProtocol_CRSF::CRSF_ADDRESS_CRSF_RECEIVER;
    _telem.ext.command.origin =
        AP_RCProtocol_CRSF::CRSF_ADDRESS_FLIGHT_CONTROLLER;
    _telem.ext.command.command_id = AP_RCProtocol_CRSF::CRSF_COMMAND_GENERAL;
    _telem.ext.command.payload[0] =
        AP_RCProtocol_CRSF::CRSF_COMMAND_GENERAL_CRSF_SPEED_RESPONSE;
...
void AP_CRSF_Telem::calc_parameter() {
    ...
    _telem.ext.param_entry.header.origin =
        AP_RCProtocol_CRSF::CRSF_ADDRESS_FLIGHT_CONTROLLER;
    size_t idx = 0;
    ...
    if (_param_request.param_num == 0) {
        ...
        for (uint8_t i = 0;
             i < AP_OSD_ParamScreen::NUM_PARAMS * AP_OSD_NUM_PARAM_SCREENS;
             i++) {
        ...
        _telem_size =
            sizeof(AP_CRSF_Telem::ParameterSettingsEntryHeader) + 1 + idx;
        _telem_type =
            AP_RCProtocol_CRSF::CRSF_FRAMETYPE_PARAMETER_SETTINGS_ENTRY;
        _pending_request.frame_type = 0;
        _telem_pending = true;
        return;
    }
    ...
    AP_OSD_ParamSetting* param = osd->get_setting(
    ...
    param->copy_name_camel_case((char*)&_telem.ext.param_entry.payload[idx],
                                17);
    ...
    switch (param->_param_type) {
        case AP_PARAM_INT8: {
            ...
            _telem.ext.param_entry.payload[idx + 1] =
                int8_t(param->_param_min);  // min
            _telem.ext.param_entry.payload[idx + 2] =
                int8_t(param->_param_max);                        // max
        ...
        case AP_PARAM_INT16: {
            ...
            put_be16_ptr(&_telem.ext.param_entry.payload[idx],
                         p->get());  // value
            put_be16_ptr(&_telem.ext.param_entry.payload[idx + 2],
                         param->_param_min);  // min
            put_be16_ptr(&_telem.ext.param_entry.payload[idx + 4],
                         param->_param_max);  // max
            put_be16_ptr(&_telem.ext.param_entry.payload[idx + 6],
                         0);  // default
            idx += 8;
            break;
        }
...
#define FLOAT_ENCODE(f) (int32_t(roundf(f)))
            put_be32_ptr(&_telem.ext.param_entry.payload[idx],
                         p->get());  // value
            put_be32_ptr(&_telem.ext.param_entry.payload[idx + 4],
                         FLOAT_ENCODE(param->_param_min));  // min
            put_be32_ptr(&_telem.ext.param_entry.payload[idx + 8],
                         FLOAT_ENCODE(param->_param_max));  // max
            put_be32_ptr(&_telem.ext.param_entry.payload[idx + 12],
                         FLOAT_ENCODE(0.0f));  // default
#undef FLOAT_ENCODE
            _telem.ext.param_entry.payload[idx + 16] = 0;  // decimal point
            put_be32_ptr(&_telem.ext.param_entry.payload[idx + 17],
                         1);  // step size
            idx += 21;
            break;
        }
        case AP_PARAM_FLOAT: {
            ...
            uint8_t digits = 0;
            const float incr =
                MAX(0.001f, param->_param_incr);  // a bug in OpenTX prevents
                                                  // this going any smaller
...
#define FLOAT_ENCODE(f) (int32_t(roundf(mult * f)))
            ...
            put_be32_ptr(&_telem.ext.param_entry.payload[idx + 17],
                         FLOAT_ENCODE(incr));  // step size
#undef FLOAT_ENCODE
            // debug("Encoding param %f(%f -> %f, %f) as %d(%d) (%d -> %d, %d)",
            // p->get(),
            //     param->_param_min.get(), param->_param_max.get(),
            //     param->_param_incr.get(), int(FLOAT_ENCODE(p->get())),
            //     digits, int(FLOAT_ENCODE(param->_param_min)),
            //     int(FLOAT_ENCODE(param->_param_max)),
            //     int(FLOAT_ENCODE(param->_param_incr)));
            idx += 21;
            break;
        }
        default:
...
#if HAL_CRSF_TELEM_TEXT_SELECTION_ENABLED
// class that spits out a chunk of data from a larger stream of contiguous
// chunks the caller describes which chunk it needs and provides this class with
// all of the data data is not written until the start position is reached and
// after a whole chunk is accumulated the rest of the data is skipped in order
// to determine how many chunks are left to be sent
class BufferChunker {
   public:
...
void AP_CRSF_Telem::calc_text_selection(AP_OSD_ParamSetting* param,
                                        uint8_t chunk) {
    const uint8_t CHUNK_SIZE = 56;
    const AP_OSD_ParamSetting::ParamMetadata* metadata =
        param->get_custom_metadata();
    ...
    _telem_size = sizeof(AP_CRSF_Telem::ParameterSettingsEntryHeader) +
                  chunker.bytes_written();
    _telem_type = AP_RCProtocol_CRSF::CRSF_FRAMETYPE_PARAMETER_SETTINGS_ENTRY;
...
// write parameter information back into AP - assumes we already know the
// encoding for floats
void AP_CRSF_Telem::process_param_write_frame(
    ParameterSettingsWriteFrame* write_frame) {
    debug("process_param_write_frame: %d -> %d", write_frame->origin,
          write_frame->destination);
    if (write_frame->destination !=
        AP_RCProtocol_CRSF::CRSF_ADDRESS_FLIGHT_CONTROLLER) {
        return;  // request was not for us
    }
#if OSD_PARAM_ENABLED
    ...
    switch (param->_param_type) {
        ...
        case AP_PARAM_INT16: {
            AP_Int16* p = (AP_Int16*)param->_param;
            if (text_selection) {
                // if we have custom metadata then the parameter is a text
                // selection
                p->set_and_save(write_frame->payload[0]);
            } else {
...
void AP_CRSF_Telem::calc_status_text() {
    if (!_statustext.available) {
        ...
        if (_crsf_version.protocol !=
            ...
            // keep only warning/error/critical/alert/emergency status text
            // messages
    ...
    _telem.bcast.custom_telem.status_text.sub_type = AP_RCProtocol_CRSF::
        CustomTelemSubTypeID::CRSF_AP_CUSTOM_TELEM_STATUS_TEXT;
    _telem.bcast.custom_telem.status_text.severity = _statustext.next.severity;
    // Note: snprintf() always terminates the string
    hal.util->snprintf(_telem.bcast.custom_telem.status_text.text,
                       AP_CRSF_Telem::PASSTHROUGH_STATUS_TEXT_FRAME_MAX_SIZE,
                       "%s", _statustext.next.text);
    // frame size = sub_type(1) + severity(1) + strlen(text) + terminator
    // Note: strlen(_telem.bcast.custom_telem.status_text.text) is safe because
    // called on a guaranteed null terminated string
...
void AP_CRSF_Telem::get_single_packet_passthrough_telem_data() {
    ...
    _telem.bcast.custom_telem.single_packet_passthrough.sub_type =
        AP_RCProtocol_CRSF::CustomTelemSubTypeID::
...
/*
 Get up to PASSTHROUGH_MULTI_PACKET_FRAME_MAX_SIZE packets of passthrough
 telemetry data (for slow links) Note: we have 2 distinct frame types (single
 packet vs multi packet) because whenever possible we use smaller frames for
 they have a higher "chance" of being transmitted by the crossfire RX scheduler.
*/
void AP_CRSF_Telem::get_multi_packet_passthrough_telem_data(uint8_t size) {
    ...
    _telem_pending = false;
    uint8_t count = 0;
    AP_Frsky_SPort::sport_packet_t
        buffer[AP_CRSF_Telem::PASSTHROUGH_MULTI_PACKET_FRAME_MAX_SIZE]{};
    // we request a PASSTHROUGH_MULTI_PACKET_FRAME_MAX_SIZE packet array, i.e. 9
    // packets
    if (!AP_Frsky_Telem::get_telem_data(buffer, count, size)) {
        return;
    }
    _telem.bcast.custom_telem.multi_packet_passthrough.sub_type =
        AP_RCProtocol_CRSF::CustomTelemSubTypeID::
            CRSF_AP_CUSTOM_TELEM_MULTI_PACKET_PASSTHROUGH;
    for (uint8_t idx = 0; idx < count; idx++) {
        _telem.bcast.custom_telem.multi_packet_passthrough.packets[idx].appid =
            buffer[idx].appid;
        _telem.bcast.custom_telem.multi_packet_passthrough.packets[idx].data =
            buffer[idx].data;
    }
    _telem.bcast.custom_telem.multi_packet_passthrough.size = count;
    _telem_size = 2 + sizeof(AP_CRSF_Telem::PassthroughMultiPacketFrame::
                                 PassthroughTelemetryPacket) *
                          count;  // subtype + size + 6*count
    _telem_type = get_custom_telem_frame_id();
    _telem_pending = true;
}
...
/*
  ...
  if is_tx_active is true then this will be a request for telemetry after
  receiving an RC frame
 */
bool AP_CRSF_Telem::_get_telem_data(AP_RCProtocol_CRSF::Frame* data,
    ...
    data->device_address =
        AP_RCProtocol_CRSF::CRSF_ADDRESS_FLIGHT_CONTROLLER;  // sync byte
 ...
 */
bool AP_CRSF_Telem::process_frame(AP_RCProtocol_CRSF::FrameType frame_type,
                                  void* data) {
    if (!get_singleton()) {
        return false;
    }
...
namespace AP {
AP_CRSF_Telem* crsf_telem() { return AP_CRSF_Telem::get_singleton(); }
};  // namespace AP
